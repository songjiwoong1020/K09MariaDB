...
/*
//방법1 : count(*) 함수를 통해 회원의 존재 유무만 판단
//		따라서 회원의 아이디, 패스워드 이외의 정보는 DB에서 가져 올 수 없다.
...
/* 
//방버2 : 회원테이블에서 레코드를 추출한 후 MemberDTO객체에 저장 후 반환받기
MemberDTO memberDTO = dao.getMemberDTO(id, pw);
//DTO객체의 name컬럼의 값이 있는지 확인...
if(memberDTO.getId() != null){
	//저장된 값이 있다면... 세션영역에 아이디, 패스워드, 이름을 속성으로 저장.
	...
	//저장된 값이 없다면... request영역에 오류메세지를 저장하고 forward한다.
	request.setAttribute("ERROR_MSG", "넌 회원이 아니시군;");
	request.getRequestDispatcher("Login.jsp").forward(request, response);
}
*/
